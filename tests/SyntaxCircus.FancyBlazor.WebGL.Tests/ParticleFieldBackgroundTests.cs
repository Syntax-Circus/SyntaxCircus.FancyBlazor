...
public sealed class ParticleFieldBackgroundTests
{
    private const string ModulePath = "./_content/SyntaxCircus.FancyBlazor.WebGL/js/fancy-blazor-webgl.js";

    [Fact]
    public void ParticleFieldBackground_MergesAttributesAndPreservesSemanticChildContent()
    {
        ...
        var cut = context.Render<ParticleFieldBackground>(parameters => parameters
            ...
            .Add(component => component.Density, -1)
            .Add(component => component.Size, 9)
            .Add(component => component.Drift, -1)
        ...
        var style = cut.Find(".syntax-circus-fancy-particle-field-background").GetAttribute("style") ?? string.Empty;

        markup.ShouldContain("syntax-circus-fancy-particle-field-background product-surface test-hook");
        markup.ShouldContain("--sc-fancy-particle-field-intensity:1");
        markup.ShouldContain("--sc-fancy-particle-field-density:0");
        markup.ShouldContain("--sc-fancy-particle-field-size:1");
        markup.ShouldContain("--sc-fancy-particle-field-drift:0");
        markup.ShouldContain("--sc-fancy-particle-field-speed:3");
        ...
        markup.ShouldContain("syntax-circus-fancy-particle-field-background__canvas");
        markup.ShouldContain("aria-hidden=\"true\"");
        markup.ShouldContain("tabindex=\"-1\"");
    }

    [Fact]
    public void ParticleFieldBackground_DisablingAfterCreation_DestroysDecorativeRuntimeAndRetainsContent()
    {
        using var context = CreateContext();
        var module = context.JSInterop.SetupModule(ModulePath);
        ...
        var cut = context.Render<ParticleFieldBackground>(parameters => parameters.Add(component => component.ChildContent, content));
    ...
    [Fact]
    public void ParticleFieldBackground_CreateCall_UsesSharedFancyBlazorDefaults()
    {
        using var context = CreateContext();
        var module = context.JSInterop.SetupModule(ModulePath);
        module.Setup<long>("createEffect", _ => true).SetResult(1);

        context.Render<ParticleFieldBackground>(parameters => parameters.Add(component => component.ChildContent, builder => builder.AddContent(0, "Readable")));
    ...
    [Fact]
    public async Task ParticleFieldBackground_RapidReenable_WaitsForTeardownBeforeCreatingReplacement()
    {
        ...
        var cut = context.Render<ParticleFieldBackground>();
    ...
    [Fact]
    public async Task ParticleFieldBackground_DisposalDuringCreation_DestroysTheStaleHandle()
    {
        await using var context = CreateContext();
        var module = context.JSInterop.SetupModule(ModulePath);
        ...
        context.Render<ParticleFieldBackground>();
    ...
    [Fact]
    public async Task ParticleFieldBackground_DisposalDuringTeardown_DoesNotCreateOrRenderAReplacement()
    {
        await using var context = CreateContext();
        var module = context.JSInterop.SetupModule(ModulePath);
