...
/// <summary>Renders a progressively enhanced refractive glass orb behind semantic child content.</summary>
public partial class RefractiveOrbBackground : ComponentBase, IAsyncDisposable
{
    ...
    /// <summary>Gets or sets the four-color orb palette.</summary>
    ...
    /// <summary>Gets or sets the orb size as a fraction of the smaller viewport dimension, from zero through one.</summary>
    [Parameter] public double Radius { get; set; } = 0.5;

    /// <summary>Gets or sets the fake-refraction lensing strength from zero through one.</summary>
    [Parameter] public double Distortion { get; set; } = 0.5;

    /// <summary>Gets or sets the Fresnel rim-highlight strength from zero through one.</summary>
    ...
    /// <summary>Gets or sets whether fine-pointer position influences the rim highlight.</summary>
    ...
    private IReadOnlyDictionary<string, object> RootAttributes => WebGlAttributeComposer.Compose(
        "syntax-circus-fancy-refractive-orb-background",
        CssClass,
        BuildStyle(),
        Style,
        ...
        {
            ["data-fancy-effect"] = "refractive-orb-background",
    ...
    private object CreateOptions() => new
    {
        ...
        radius = WebGlAttributeComposer.Clamp(Radius, 0, 1),
        distortion = WebGlAttributeComposer.Clamp(Distortion, 0, 1),
    ...
    private string BuildStyle() =>
        $"--sc-fancy-primary:{Palette.Primary};--sc-fancy-secondary:{Palette.Secondary};--sc-fancy-accent:{Palette.Accent};--sc-fancy-background:{Palette.Background};--sc-fancy-refractive-orb-intensity:{WebGlAttributeComposer.Number(WebGlAttributeComposer.Clamp(Intensity, 0, 1))};--sc-fancy-refractive-orb-radius:{WebGlAttributeComposer.Number(WebGlAttributeComposer.Clamp(Radius, 0, 1))};--sc-fancy-refractive-orb-distortion:{WebGlAttributeComposer.Number(WebGlAttributeComposer.Clamp(Distortion, 0, 1))};--sc-fancy-refractive-orb-sheen:{WebGlAttributeComposer.Number(WebGlAttributeComposer.Clamp(Sheen, 0, 1))};--sc-fancy-refractive-orb-speed:{WebGlAttributeComposer.Number(WebGlAttributeComposer.Clamp(Speed, 0, 3))}";
    ...
    private async ValueTask<bool> CreateOrUpdateAsync()
    {
        ...
        if (_handle is null)
        {
            var createdHandle = await Runtime.CreateAsync(_element, "refractive-orb-background", options).ConfigureAwait(false);
